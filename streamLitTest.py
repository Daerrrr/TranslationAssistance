# src/streamlit_app/day15_app.py
...
with col_output:
    ...
    if 'translation_result' in st.session_state:
        ...
        st.info("👈 请在左侧输入Java代码并点击翻译按钮")

        # 显示示例翻译结果
        st.caption("示例预览：")
        example_result = '''# 计算器类 - 从Java翻译到Python
class Calculator:
    ...
    def multiply(self, a: int, b: int) -> int:
        """乘法运算，使用循环调用add方法实现"""
        result = 0
        for i in range(b):  # Python的range从0开始，不包括b
...
with st.sidebar:
    ...
    max_tokens = st.slider("最大token数", 100, 5000, 2000, 100)
    ...
    with st.expander("如何获取API密钥"):
        ...
        st.write("2. 注册并登录账户")
        st.write("3. 在API密钥页面生成新密钥")

    with st.expander("支持的功能"):
        st.write("✅ 单文件Java代码翻译")
        st.write("✅ 多种目标语言支持")
        st.write("✅ 代码注释生成")
        st.write("🔄 批量翻译（后续版本）")
    ...
    # 版本信息
    st.markdown("---")
    st.caption("版本 v1.0 · 基于LangChain + DeepSeek")
    st.caption("Java工程师AI转型学习计划 · Week3 Day1")

# 页脚信息
st.markdown("---")
st.caption("🔧 技术栈：Streamlit + LangChain + DeepSeek API")
st.caption("📚 学习目标：掌握AI应用Web界面快速开发")